         ...
         for i in range(len(mbox_loc)):
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m])>0:
                    ...
                    #soft_nms
                    #按照存在物体的置信度排序
                    # _,conf_sort_idx=torch.sort(confs_to_process,descending=True)
                    # boxes_to_process=boxes_to_process[conf_sort_idx]
                    # confs_to_process=confs_to_process[conf_sort_idx]
                    #进行非极大抑制
                    '''
                    good_boxes=[]
                    confs=[]
                    while boxes_to_process.size(0):
                        # 取出这一类置信度最高的，一步一步往下判断，判断重合程度是否大于nms_thres，如果是则去除掉

                        _,conf_sort_idx=torch.sort(confs_to_process,descending=True)
                        boxes_to_process=boxes_to_process[conf_sort_idx]
                        confs_to_process=confs_to_process[conf_sort_idx]
                        good_boxes.append(boxes_to_process[0].unsqueeze(0))
                        confs.append(confs_to_process[0].unsqueeze(0))

                        # 置信度衰减
                        if len(boxes_to_process)==1:
                            break

                        boxes_to_process=boxes_to_process[1:]
                        confs_to_process=confs_to_process[1:]
                        ious=bbox_iou(good_boxes[-1],boxes_to_process)

                        ge_threshod_bool = ious>= nms_iou
                        confs_to_process[ge_threshod_bool]*= (1. - ious[ge_threshod_bool]) 

                        # confs_to_process=torch.exp(-(ious*ious)/0.5)*confs_to_process

                        # boxes_to_process=boxes_to_process[1:]
                        # confs_to_process=confs_to_process[1:]


                        c_confs_m =confs_to_process>=confidence
                        # 取出得分高于confidence的框
                        boxes_to_process = boxes_to_process[c_confs_m]
                        confs_to_process = confs_to_process[c_confs_m]

                        argsort=torch.argsort(confs_to_process,descending=True)

                        boxes_to_process=boxes_to_process[argsort]
                        confs_to_process=confs_to_process[argsort]

                    labels      = (c - 1) * torch.ones((len(confs), 1)).cuda() if mbox_conf.is_cuda else (c - 1) * torch.ones((len(confs), 1))
                    good_boxes=torch.cat(good_boxes)
                    confs=torch.cat(confs)[:, None]
                    '''
